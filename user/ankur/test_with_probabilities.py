...
def preprocess():
    ...
    model = Sequential()
    model.add(LSTM(512, input_shape=(n_p, 131), return_sequences=True))
    ...
    # model.add(Dense(256))
    model.add(Dropout(0.3))
    model.add(Dense(131, activation="softmax"))  # output=3

    model.compile(loss="categorical_crossentropy", optimizer="RMSprop", metrics=["accuracy"])
    model.fit(x, y, epochs=50, batch_size=200, validation_split=0.1)
